...
const App = () => {
  ...
  useEffect(() => {
    const stored = JSON.parse(window.localStorage.getItem("cities"));
    if (stored) setCities(stored);
  }, []);
  ...
  return (
    <div className={styles.appContainer}>
      <div className={styles.app}>
        ...
        <Search
          onSubmit={(city) =>
            cities.includes(city) || setCities([...cities, city])
          }
        />
        {cities.map((city) => {
          return <City location={city} key={city} deleteCity={deleteCity} />;
        })}
      </div>
    </div>
  );
